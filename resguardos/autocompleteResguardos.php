...
if ($con)
{
	$fetch = mysqli_query($con,"SELECT * FROM  inv_pcs WHERE resguardo = 'SI' AND Equipo_Serie like '%" . mysqli_real_escape_string($con,($_GET['term'])) . "%' LIMIT 0 ,50"); 	
	/* Recuperar y almacenar en conjunto los resultados de la consulta.*/
	//printf("Error: %s\n", mysqli_error($con));

	while ($row = mysqli_fetch_array($fetch)) {
        // quitar esto
      if($row['resguardoUnico']=='NO'){
